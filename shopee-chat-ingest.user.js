...
(function () {
  ...
  const _wsProbe = { n: 0, samples: [], texts: [], others: [], kinds: {}, types: {}, skip: {}, skipSample: {}, api: {}, apiBody: [], at: 0, hooked: [] };
  ...
  function wsIngest(str) {
    // 取り込まなかった理由を数える＋**実物を1つずつ残す**（推測で実装して壊さないため）
    const why = (k, sample) => {
      _wsProbe.skip[k] = (_wsProbe.skip[k] || 0) + 1;
      try {
        if (sample && !_wsProbe.skipSample[k] && Object.keys(_wsProbe.skipSample).length < 8) {
          _wsProbe.skipSample[k] = String(sample).slice(0, 900);
        }
      } catch (_) {}
    };
    let o; try { o = JSON.parse(str); } catch (_) { why('JSONでない'); return; }
    const pay = o.payload || o;
    if (String(pay.message_type || '') !== 'message') { why('message以外'); return; }
    let mc; try { mc = JSON.parse(pay.message_content); } catch (_) { why('中身が壊れている'); return; }
    if (!mc || !mc.id) { why('IDなし'); return; }
    ...
    if (!buyer || /^gcsonlinestore/i.test(buyer)) { why('相手が特定できない'); return; }
    ...
    else { why('未対応の型:' + ty, JSON.stringify(mc)); return; }   // 未知の型は取り込まない（実物を残す）
    if (!text) { why('本文が空:' + ty, JSON.stringify(mc)); return; }
    ...
    if (seenMsg.has(id)) { why('取り込み済み'); return; }
    seenMsg.add(id);
  ...
  function domHeaderInfo() {
    ...
    [].slice.call(document.querySelectorAll('div,span,a')).forEach(el => {
      ...
      if (r.top >= 70 && r.top <= 180 && r.left >= tr.left - 20 && r.left <= tr.left + 300 && el.children.length === 0
        ...
        && !/^(Yesterday|Today|Just now|Now|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday|Closed)$|^\d{1,2}\/\d{1,2}(\/\d{2,4})?$|^\d{1,2}\s[A-Za-z]{3,9}$|^[A-Za-z]{3,9}\s\d{1,2}$/i.test(t)   // ★3.44.3 日付区切り（"Yesterday"/"19 Jun"/"DD/MM"）を名前に拾わない（実測：buyer="Yesterday" が chat_messages に5行）
  ...
  // ★一覧スキャンは442件を上から下までスクロールするので、頻繁だと**画面がほぼ常時動いて見える**
  //   （巡回を止めても動きが消えなかった原因はこれ・本人が「一覧スキャン完了」の表示で発見）。
  //   ポータルのメッセージ画面を開いた時にも走る(rescan_list)ので、自動は20分に1回で十分。
  // ★起動時の自動スキャンは廃止（v3.43.0）。リロードのたびに一覧を全件スクロールして
  //   「まだ動いている」状態になっていた。ポータルのメッセージ画面を開いた時に走る(rescan_list)ので不要。
  setInterval(() => { if (isWorker() && !cycling && !scanning && !userBusy()) scanAllConversations(false); }, 20 * 60000);
