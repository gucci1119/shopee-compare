...
function researchJan_(names, hw) {
  ...
  var prompt =
    ...
    '\n★重要：商品名は【出品用の英語表記】です。中身は日本で売られた商品のことが多く、その場合は\n' +
    '　そのまま英語で検索してもJANが出てきません。**日本語の正式タイトルが分かるものは、日本語で検索してください**。\n' +
    '　ただし【全部が日本語商品とは限りません】。海外版・輸入版・日本語名が存在しない商品（雑貨・海外ブランド等）は、\n' +
